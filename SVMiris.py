from sklearn import datasets  #to get the iris set
...
def visualize_sepal_data():
    ...
    X = iris.data[:, :2]  # we only take the first two features - !SEPAL! LENGTH & WIDTH
    print(X)
    y = iris.target
...
def visualize_petal_data():
    iris = datasets.load_iris()
    X = iris.data[:, 2:]  # we only take the last two features - !PETAL! LENGTH & WIDTH
    y = iris.target
    plt.scatter(X[:, 0], X[:, 1], c=y, cmap=plt.cm.coolwarm)
